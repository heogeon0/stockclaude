...
- `mcp/server.py` — FastMCP entry. 88 `@mcp.tool` 단일 파일 (4250줄 — 분할은 §10.2 이슈로 추적).

## 단일 유저 fallback (research §4.9)
...
| `jobs/` | 배경 작업 (현재 진입점 미상 — §10.6) | 있음 |
| `schemas/` | pydantic 응답 모델 (frontend types 수동 동기화 §10.5) | 있음 |
