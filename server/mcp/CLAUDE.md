...
`server/mcp/server.py`는 **4250줄, 88 `@mcp.tool` 데코레이터**가 한 파일에 모두 등록되어 있음. 분할은 #12 이슈로 추적 — **즉시 분할 금지**. 신규 툴은 의미상 가장 가까운 그룹의 섹션 주석 아래에 추가.
...
분할 시 권장 (#12 이슈에서 다룸): `tools/{queries,analysis,writes,market,financials,consensus,review_strategy}.py`로 그룹별 분리, `server.py`는 import + FastMCP 등록만 보유. 즉시 작업 X.
