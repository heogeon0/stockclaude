# Per-Stock Analysis — 종목 1건 분석 단일 진입점 (5단계, v6 단순화 + 라운드 2026-05-daily-workflow-tightening)
...
> 라운드 2026-05-daily-workflow-tightening: stale 체크는 **종목 영역만** (`scope="stock", code=code` — economy 제외, daily Phase 2 가 처리). step 4 WebSearch 1회 BLOCKING 복원 (Tier 1 + Tier 4 KR이면).
...
- **WebSearch BLOCKING ⛔** — step 4 LLM 종합 판단 시 1회/종목 의무 호출. Tier 1 글로벌 매체 (Bloomberg/Reuters/FT/WSJ) + KR 종목이면 Tier 4 (한경/매경/이데일리/더벨) 추가. 결과 인용 시 `(도메인, YYYY-MM-DD)` 의무. 도메인 화이트리스트 단일 출처 → `websearch-domains.md`.
...
```python
fresh = check_base_freshness(scope="stock", code=code)
# economy 제외 — daily Phase 2 가 자기 영역 책임 (라운드 2026-05-daily-workflow-tightening)
# 응답: industries[] (해당 종목의 industry_code 행) + stocks[] (해당 종목 코드 행)
```

- 응답에서 stale 여부 확인:
  - `industries[]` 중 해당 종목의 industry_code 행
  - `stocks[]` 중 해당 종목 코드 행
- ⚠️ economy stale 확인은 본 절차 밖 — daily Phase 2 (`scope="economy"`) 또는 research/discover 진입 시 별도 호출.

### 2단계 — stale 갱신 (cascade industry → stock, economy 제외)

stale 차원에 대해 **cascade 순서 (industry → stock)** 로 inline 절차 실행:

| 차원 | 절차서 |
|---|---|
| industry stale | `references/base-industry-update-inline.md` (`compute_industry_metrics` + WebSearch BLOCKING — `websearch-rules.md` 매트릭스) |
| stock stale | `references/base-stock-update-inline.md` (정형 MCP 우선 + 딜레이더 6종) |

각 inline 절차 완료 후 read-back 검증 의무 (`updated_at` 갱신 확인).

> economy stale → daily Phase 2 의 `base-economy-update-inline.md` 진입점에서 처리 (per-stock cascade 외부).
...
### 4단계 — LLM 종합 판단 (WebSearch 1회/종목 BLOCKING ⛔)

3단계 응답 (base 본문 3층 + 정량 raw + disclosures + insider) + **회고 학습 + 거장 원칙 + WebSearch 1회 BLOCKING** 종합.

#### WebSearch 1회 BLOCKING (라운드 2026-05-daily-workflow-tightening)

종목 1건마다 의무 1회. 누락 시 보고서 최상단 ⚠️ "WebSearch BLOCKING 위반".

- **Tier 1** (글로벌 매체 — 의무): Bloomberg / Reuters / FT / WSJ / Nikkei / CNBC
- **Tier 4** (KR 종목이면 추가): 한경 / 매경 / 이데일리 / 더벨 / 머니투데이
- 권장 쿼리:
  - KR: `site:bloomberg.com OR site:reuters.com OR site:hankyung.com OR site:mk.co.kr {종목명} YYYY-MM-DD`
  - US: `site:bloomberg.com OR site:reuters.com OR site:wsj.com {ticker} YYYY-MM-DD news earnings`
- 결과 인용 시 본문에 `(도메인, YYYY-MM-DD)` 의무.
- 화이트리스트 도메인 매치 0건이면 쿼리 재시도 (`site:` 한정 더 좁힘). 결과 quality 가드는 `websearch-rules.md`.
- 도메인 단일 출처 → `references/websearch-domains.md`.
...
6. WebSearch 1회 결과 (Tier 1 + Tier 4 KR이면) — 매체 톤 / 시장 반응 / 정형 미커버 nuance
```

> 라운드 2026-05-daily-workflow-tightening 변경: v6 자율 정책 회귀. 정형 90% 커버는 *수치*만, 매체 톤·지정학·시장 반응 같은 nuance 는 search 필수. 자율은 효율 추구로 누락 빈발 → BLOCKING 복원 + 도메인 화이트리스트 의무.
