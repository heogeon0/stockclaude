...
추가 — **딜 레이더 6종 (정형 우선 → #3 / #4 는 WebSearch BLOCKING)**: → `~/.claude/skills/stock/references/deal-radar-checklist.md` 참조.

| # | 카테고리 | 정형 MCP (1차) | WebSearch BLOCKING |
|---|---|---|---|
| 1 | Pending M&A | `get_kr_disclosures` (주요사항보고서·영업양수도결정·합병결정) / `get_us_disclosures` (8-K) | (정형 우선 — 정형 미커버 시 자율) |
| 2 | 관계사·모회사·자회사 딜 | `get_kr_disclosures` (종속회사변경·영업양수도) | (정형 우선 — 정형 미커버 시 자율) |
| 3 | 경쟁사 딜 | — (외부 산업 동향) | **search 1 BLOCKING** (Tier 1 — Bloomberg / Reuters / FT / WSJ / Nikkei) |
| 4 | 규제·반독점 | `get_kr_disclosures` (소송제기·우발채무) 부분 | **search 2 BLOCKING** (Tier 2 공식 기관 우선 — sec.gov / fsc.go.kr / fss.or.kr / europa.eu 등) |
| 5 | 주주행동주의·분사 | `get_us_insider_trades` (대규모 매도 패턴) + `get_kr_disclosures` (분할결정·자사주) | (정형 우선 — 정형 미커버 시 자율) |
| 6 | 대주주 지분 변동 | **`get_kr_insider_trades` / `get_us_insider_trades` 우선** + `get_kr_major_shareholders` | (정형 우선 — 정형 미커버 시 자율) |

### #3 경쟁사 — WebSearch 1회 BLOCKING (Tier 1)

> 화이트리스트는 `~/.claude/skills/stock/references/websearch-domains.md` 참조. Tier 1 글로벌 매체 한정.

표준 쿼리 (예시):
```
site:bloomberg.com OR site:reuters.com OR site:ft.com OR site:wsj.com {경쟁사명 또는 산업} M&A YYYY
site:nikkei.com OR site:cnbc.com {산업} 경쟁 동향 YYYY-MM
```

### #4 규제 — WebSearch 1회 BLOCKING (Tier 2 공식 기관 우선)

표준 쿼리 (예시):
```
site:sec.gov OR site:fsc.go.kr OR site:fss.or.kr {ticker / 종목명} regulation YYYY
site:europa.eu OR site:moef.go.kr {산업} 정책 YYYY
```

> Tier 2 공식 기관 우선 매치. 공식 기관 자료가 없을 때만 Tier 1 (Bloomberg / Reuters) 보조 인용 허용. 두 search (#3 / #4) 모두 누락 시 보고서 최상단 `⚠️ WebSearch BLOCKING 위반` 명시.

### 그 외 딜레이더 (#1 M&A / #2 관계사 / #5 행동주의 / #6 대주주 변동)

정형 MCP 1차 우선. 정형으로 답이 나오면 search skip OK. 정형 미커버 (루머·시장 추측) 시에만 LLM 자율 search (BLOCKING X).
...
- **WebSearch 결과 인용 시 도메인 + 날짜 명시 의무** — 예: `(Reuters, 2026-05-04)`, `(SEC EDGAR 8-K, 2026-04-29)`. 딜레이더 #3·#4 본문에 인라인.
...
- [ ] **#3 경쟁사 WebSearch 1회 BLOCKING 충족** (Tier 1 — Bloomberg / Reuters / FT / WSJ / Nikkei)
- [ ] **#4 규제 WebSearch 1회 BLOCKING 충족** (Tier 2 공식 기관 우선 — sec.gov / fsc.go.kr / fss.or.kr / europa.eu)
- [ ] **인용 도메인·날짜 명시** — 모든 WebSearch 인용에 `(매체, YYYY-MM-DD)` 동반. 화이트리스트 외 도메인 사용 금지 (`websearch-domains.md` Tier 1~4 매치).
