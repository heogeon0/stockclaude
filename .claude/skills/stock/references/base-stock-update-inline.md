...
추가 — **딜 레이더 6종 (정형 우선 → WebSearch 보조)**: → `~/.claude/skills/stock/references/deal-radar-checklist.md` 참조.

| # | 카테고리 | 정형 MCP (1차) | WebSearch 권장 (보조) |
|---|---|---|---|
| 1 | Pending M&A | `get_kr_disclosures` (주요사항보고서·영업양수도결정·합병결정) / `get_us_disclosures` (8-K) | M&A 루머·협상 단계 |
| 2 | 관계사·모회사·자회사 딜 | `get_kr_disclosures` (종속회사변경·영업양수도) | 비공식 보도 |
| 3 | 경쟁사 딜 | — (외부 산업 동향) | `site:bloomberg.com OR site:reuters.com` 산업 M&A |
| 4 | 규제·반독점 | `get_kr_disclosures` (소송제기·우발채무) 부분 | `site:fsc.go.kr OR site:sec.gov` 조사·과징금 |
| 5 | 주주행동주의·분사 | `get_us_insider_trades` (대규모 매도 패턴) + `get_kr_disclosures` (분할결정·자사주) | 행동주의 펀드 공개 캠페인 |
| 6 | 대주주 지분 변동 | **`get_kr_insider_trades` / `get_us_insider_trades` 우선** + `get_kr_major_shareholders` | 대량보유 5% 룰 외 시장 추측 |

> 강제 호출 횟수 X — 정형으로 답이 나오면 search skip OK. 6종 카테고리 중 정형 미커버(루머·시장 추측·경쟁사 동향)만 자율 호출.
